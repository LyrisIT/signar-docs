...
::: tip IMPORTANTE
Disponible para clientes con el módulo de Aspectos De Documentos hablilitado.
:::
...
<br>
<ClientOnly>
  <ImageCompare left-url="/images/v12/vista-permisos-antes.png" right-url="/images/v12/vista-permisos-nuevo.png" description="A la izquierda se encuentra la versión anterior, a la derecha, la nueva. Podes comparar utilizando la barra central."/>
</ClientOnly>
...
- SIGNAR-222 (H025) Modificar metadatos a un documento: Nueva versión donde presentamos mejoras de UX/UI en el panel de metadatos de un documento o de una unidad compuesta. Ahora es mas sencillo poder hacer la modificacion de metadatos de un documento.
...
- SIGNAR-279 (H065) Nuevo modulo: Auditoria. Ahora las organizaciones pueden tener seguimiento de todo lo que ocurre con sus usuarios y documentos. Debe contactar a Lyris IT para poder contar con el modulo.
- SIGNAR-280 (H066) Nueva vista: Auditorias. Se pueden ver las pistas de auditoria de los usuarios. Allí se listan en orden cronológico descendente todas las acciones registradas realizadas por los usuarios de la organización. 
Es posible filtrar por tipo de acción, rango de fechas, y usuarios.
- SIGNAR-289 Al mover documentos, en relacion a SIGNAR-271 se debe contemplar el bloqueo de permisos (el forzar herencia de los permisos) en la carpeta origen. Si el origen fuerza la herencia hacia abjao, no se permite mover el documento. Tambien se contempla si la carpeta destino cuenta con permisos bloqueados: El documento comenzará a heredar los permisos de su nueva carpeta si la carpeta destino fuerza la herencia hacia abajo
...
- SIGNAR-270 Si se intenta crear carpetas o documentos en una carpeta donde se fuerzan la herencia hacia abajo, por default las nuevas carpetas o nuevos documentos heredaran permisos (La opcion no se puede cambiar en los modals de crear nueva carpeta o subir documentos del repositorio)
...
- SIGNAR-279 UX/UI: Agregado link para poder descargar aplicacion de android si se encuentran en mobile.
- SIGNAR-284 UX/UI Bugfix: Al editar una serie, al cambiar el nombre de un estado, no habia cambio en la etiqueta del nombre, seguia apareciendo el nombre anterior. Esto fue solucionado.
- SIGNAR-285 Bugfix: Al eliminar una serie documental del cual existen unidades compuestas, el sistema devolvia error. Ahora se verifica si existen subunidades en las unidades compuestas asociadas a la serie y con que exista una, falla el metodo. Los usuarios deben si o si eliminar cada unidad compuesta con subunidades cargadas si desean perder los documentos asociados a esa unidad compueta. De todas formas, las unidades compuestas sin subunidades se borrarian sin problemas, sin consideracion de si se ejecuto algun evento.
- SIGNAR-299 UX/UI Bugfix: Al crear subunidades de una serie documental, si se saca el tilde en el checkbox de tipo de subunidad "PDF" al crear una nueva subunidad de un documento y se agregaba, daba error. Esto fue solucionado deshabilitando la posibilidad de cambiar el valor de este checkbox ya que PDF por ahora es el unico tipo de dato que permitimos.
- SIGNAR-303 UX/UI Bugfix: Al crear un nuevo permiso para un area, el autocomplete no estaba obedeciendo la busqueda por nombre.
- SIGNAR-304 UX/UI Bugfix: Fix de un error interno del repositorio al intentear modificar permisos de una carpeta cuando se tiene la carpeta abierta.
- SIGNAR-306 UX/UI Bugfix: En la vista de documento, si se pierden los permisos de un documentos y el usuario presionaba el boton "Recargar", el usuario se quedaba viendo el documento pero en realidad no se actualizan los datos porque se dejo de tener acceso. Ahora redirige a un 404.
- SIGNAR-306 UX/UI Bugfix: Si se intentaba entrar modificando la URL a otro documento, el navegador no respondia, esto pasaba por querer acceder a un documento del cual el usuario no tiene acceso. Ahora redirige a un 404.
- UX/UI: Nueva versión del componente para administrar los permisos de un documento o carpeta. Ahora la informacion es mas facil de leer y mas amigable.
- UX/UI: Administracion de permisos: Al agregar un nuevo permiso para un area o usuario, el slider para elegir los permisos es mas amigable y se entiende mejor los permisos que se estan asignando.
- UX/UI Bugfix: Ahora no se ve mas el boton de asociar aspectos si no existen mas aspectos para asociar al documento (o sea, se ocupan todos los aspectos disponibles).
- UX/UI Bugfix: Algunos usuarios podian experimentar problemas de permisos sobre un documento, lo cual, ahora el usuario deberia contar con el permiso que le corresponde.
- UX/UI Bugfix: Al eliminar una serie en la vista de "Ver serie", la vista quedaba en pantalla y no se redirigia al listado de series documentales.
- UX/UI Bugfix: En un metadato, al agregar un numero con “,” a un atributo del tipo “numero entero” o “numero decimal”, el campo lo trunca a entero. Se corrigio esta experiencia
